...
func TestSetClassMemoVisibility(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, apiv1.ClassVisibility_CLASS_PRIVATE, visibility2.GetVisibility())
...
func TestGetClassMemoVisibility(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, apiv1.ClassVisibility_CLASS_PRIVATE, gotUpdatedVisibility.GetVisibility())
}
...
func TestListClassMemoVisibilities(t *testing.T) {
	...
	memoVisibilities := []struct {
		content    string
		visibility apiv1.ClassVisibility
	}{
		{"Memo 1 content", apiv1.ClassVisibility_CLASS_PUBLIC},
		{"Memo 2 content", apiv1.ClassVisibility_CLASS_PRIVATE},
		{"Memo 3 content", apiv1.ClassVisibility_CLASS_PUBLIC},
		{"Memo 4 content", apiv1.ClassVisibility_CLASS_PRIVATE},
		{"Memo 5 content", apiv1.ClassVisibility_CLASS_PUBLIC},
	}

	for _, mv := range memoVisibilities {
...
func TestCreateClassTagTemplate(t *testing.T) {
	...
	_, err = ts.Service.CreateClassTagTemplate(teacherCtx, &apiv1.CreateClassTagTemplateRequest{
		...
		TagTemplate: &apiv1.ClassTagTemplate{
			...
			DisplayName: "Math Problems",   // Same display name - should trigger duplicate
	...
	// Test 3: Create with empty color (should succeed)
	tagTemplate2, err := ts.Service.CreateClassTagTemplate(teacherCtx, &apiv1.CreateClassTagTemplateRequest{
		Class: createdClass.GetName(),
		TagTemplate: &apiv1.ClassTagTemplate{
			Name:        "tag-template-2",
			DisplayName: "Empty Color Template",
			Description: "Template with no color",
			Color:       nil,
		},
	})
	require.NoError(t, err)
...
func TestUpdateClassTagTemplate(t *testing.T) {
	...
	colorValue2 := "#33FF57"
	updateMask1 := &fieldmaskpb.FieldMask{Paths: []string{"display_name", "description", "color"}}
	...
	colorValue3 := "#000000"
	updateMask2 := &fieldmaskpb.FieldMask{Paths: []string{"display_name"}}
	_, err = ts.Service.UpdateClassTagTemplate(teacherCtx, &apiv1.UpdateClassTagTemplateRequest{
		...
		UpdateMask: updateMask2,
	})
	require.Error(t, err)

	// Test 3: Update with partial mask (only display_name)
	updateMask3 := &fieldmaskpb.FieldMask{Paths: []string{"display_name"}}
	updatedPartial, err := ts.Service.UpdateClassTagTemplate(teacherCtx, &apiv1.UpdateClassTagTemplateRequest{
		TagTemplate: &apiv1.ClassTagTemplate{
			...
			Color:       &colorValue3,         // Not in update_mask
		},
	...
	colorValue4 := "#FF0000"
	updateMask4 := &fieldmaskpb.FieldMask{Paths: []string{"display_name"}}
...
func TestListClassTagTemplates(t *testing.T) {
	...
	tagTemplates := []struct {
		...
		{"math-template", "Math Problems", "Template for math", "#FF5733"},
		{"science-template", "Science Topics", "Template for science", "#33FF57"},
		{"english-template", "English Literature", "Template for English", "#3357FF"},
		{"history-template", "History Events", "Template for history", "#FF33A1"},
		{"art-template", "Art Topics", "Template for art", "#A133FF"},
	}
	...
	// Verify all templates are present by display name
	foundDisplayNames := make(map[string]bool)
	for _, tt := range listResp.GetTagTemplates() {
		foundDisplayNames[tt.GetDisplayName()] = true
	}
	for _, tt := range tagTemplates {
		require.True(t, foundDisplayNames[tt.displayName], "Should find template: "+tt.displayName)
	}
