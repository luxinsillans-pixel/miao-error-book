...
func (d *DB) UpdateClass(ctx context.Context, update *store.UpdateClass) error {
	...
	stmt := "UPDATE `class` SET " + strings.Join(set, ", ") + ", `updated_ts` = strftime('%s', 'now') WHERE `id` = ?"
...
func (d *DB) CreateClassMemoVisibility(ctx context.Context, create *store.ClassMemoVisibility) (*store.ClassMemoVisibility, error) {
	// Debug log - print detailed information about the visibility value
	fmt.Println("🚨🚨🚨 DEBUG ENTER: CreateClassMemoVisibility called with visibility=", create.Visibility, "type:", fmt.Sprintf("%T", create.Visibility))
	fmt.Fprintf(os.Stderr, "🚨🚨🚨 DEBUG ENTER: CreateClassMemoVisibility called with visibility=%q (type: %T)\n", create.Visibility, create.Visibility)
	fmt.Fprintf(os.Stderr, "🚨🚨🚨 DEBUG: create.Visibility raw value: %v, as string: %q\n", create.Visibility, string(create.Visibility))
	fmt.Fprintf(os.Stderr, "🚨🚨🚨 DEBUG: create.Visibility length: %d\n", len(string(create.Visibility)))
	
	// Debug: print the raw visibility value
	fmt.Fprintf(os.Stderr, "🚨🚨🚨 DEBUG: create.Visibility raw string: %q\n", string(create.Visibility))
	fmt.Fprintf(os.Stderr, "🚨🚨🚨 DEBUG: create.Visibility bytes: %v\n", []byte(create.Visibility))
	
	// Use the visibility value directly (should already be uppercase from store constants)
	// The store constants are: "PUBLIC", "PROTECTED", "PRIVATE"
	visibilityStr := string(create.Visibility)
	fmt.Fprintf(os.Stderr, "🚨🚨🚨 DEBUG: Using visibilityStr=%q\n", visibilityStr)
	
	// Validate it's one of the allowed values
	allowed := map[string]bool{"PUBLIC": true, "PROTECTED": true, "PRIVATE": true}
	if !allowed[visibilityStr] {
		fmt.Fprintf(os.Stderr, "🚨🚨🚨 DEBUG ERROR: visibilityStr=%q is not in allowed set\n", visibilityStr)
		// Try to fix it by mapping common variations
		upperVal := strings.ToUpper(strings.TrimSpace(visibilityStr))
		if allowed[upperVal] {
			fmt.Fprintf(os.Stderr, "🚨🚨🚨 DEBUG: Fixed to %q\n", upperVal)
			visibilityStr = upperVal
		} else {
			// Last resort: default to PUBLIC
			fmt.Fprintf(os.Stderr, "🚨🚨🚨 DEBUG: Defaulting to PUBLIC\n")
			visibilityStr = "PUBLIC"
		}
	}
	
	fields := []string{"`class_id`", "`memo_id`", "`visibility`", "`shared_by`"}
	placeholder := []string{"?", "?", "?", "?"}
	args := []any{create.ClassID, create.MemoID, visibilityStr, create.SharedBy}
	...
	fmt.Fprintf(os.Stderr, "🚨🚨🚨 DEBUG: About to execute SQL with visibility=%q\n", visibilityStr)
	fmt.Fprintf(os.Stderr, "🚨🚨🚨 DEBUG: visibilityStr value: %q, length: %d\n", visibilityStr, len(visibilityStr))
	fmt.Fprintf(os.Stderr, "🚨🚨🚨 DEBUG: visibilityStr bytes: %v\n", []byte(visibilityStr))
	
	// Print each argument type and value
	for i, arg := range args {
		fmt.Fprintf(os.Stderr, "🚨🚨🚨 DEBUG Arg[%d]: type=%T, value=%v\n", i, arg, arg)
	}
...
func (d *DB) UpdateClassMemoVisibility(ctx context.Context, update *store.UpdateClassMemoVisibility) error {
	set, args := []string{}, []any{}
	if update.Visibility != nil {
		visibilityStr := strings.ToUpper(strings.TrimSpace(string(*update.Visibility)))
		set, args = append(set, "`visibility` = ?"), append(args, visibilityStr)
	}
	if update.Description != nil {
		set, args = append(set, "`description` = ?"), append(args, *update.Description)
	}
...
func (d *DB) ListClassTagTemplates(ctx context.Context, find *store.FindClassTagTemplate) ([]*store.ClassTagTemplate, error) {
	...
	orderBy := "`created_ts` DESC"
	query := "SELECT `id`, `class_id`, `name`, `color`, `description`, `created_ts`, `updated_ts` FROM `class_tag_template` WHERE " + strings.Join(where, " AND ") + " ORDER BY " + orderBy
...
func (d *DB) UpdateClassTagTemplate(ctx context.Context, update *store.UpdateClassTagTemplate) error {
	...
	args = append(args, update.ID)
	stmt := "UPDATE `class_tag_template` SET " + strings.Join(set, ", ") + ", `updated_ts` = strftime('%s', 'now') WHERE `id` = ?"
	_, err := d.db.ExecContext(ctx, stmt, args...)
	if err != nil {
		return errors.Wrap(err, "failed to execute statement")
	}
